...
module.exports = function() {
    ...
    window.Element.prototype.isVisible = function() {
        ...
        //> This is required to find out how the browser actually measures document.elementFromPoint
        //> Opera and Safari take absolute values while IE > 9 and FF, Chrome take relative values
        //> The !! bang operator ensures a proper Boolean is returned
        var x = (window.pageXOffset) ? (window.pageXOffset + window.innerWidth - 1) : 0;
        var y = (window.pageYOffset) ? (window.pageYOffset + window.innerHeight - 1) : 0;

        //> check if the browser takes relative (or) absolute dimensions
        var isRelative = !!((!x && !y) || !document.elementFromPoint(x, y));
        ...
        //> if its relative measures, then there is no need to include scroll amount as browser always calculates relatively
        //> If its not relative, then there is a need to include scroll amount
        if (isRelative) {
            if (!inside(document.elementFromPoint(rect.left + _this.offsetWidth / 2, rect.top + _this.offsetHeight / 2), _this)) {
                return false; //> element not visible
            }
        }else {
            if (!inside(document.elementFromPoint(rect.left + _this.offsetWidth / 2 + window.pageXOffset, rect.top + _this.offsetHeight / 2 + window.pageYOffset), _this) || isOutsideViewPort()) {
                return false; //> element not visible
            }
        }
