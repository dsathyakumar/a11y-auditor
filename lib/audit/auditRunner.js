...
var _resultObj = {}, // returns the results of the audit run
    ...
    _mode = '', //mode in which the auditRunner is working
    _displayOptions; // display options for final result

//> defining variables before using - jshint ;)
var _executeGlobalRuleAudits, _conductAudit, _processHTMLObj, _isSkipRule, _populateErrors, _handleResult;
...
//> this function handles the result based on the displayOptions
_handleResult = function _handleResult(resultObj) {
    var _mustPopulateResult = false;
    if (_displayOptions === enums.displayEnum.ERR) {
        _mustPopulateResult = (resultObj.TYPE === enums.displayEnum.ERR);
    }else if (_displayOptions === enums.displayEnum.WARN) {
        _mustPopulateResult = (resultObj.TYPE === enums.displayEnum.WARN);
    }else {
        _mustPopulateResult = (resultObj.TYPE === enums.displayEnum.ERR || resultObj.TYPE === enums.displayEnum.WARN);
    }
...
_isSkipRule = function _isSkipRule(rule, elem) {
    ...
    if (_mode === enums.modeEnum.RUNONLY)
        _isSkip = !_isSkip;
    return _isSkip;
};
